...
import { SkillsData, DataState } from '../models';

@Injectable({ providedIn: 'root' })
export class SkillsService {
  private http = inject(HttpClient);

  private state = signal<DataState<SkillsData>>({
    data: null,
    state: 'idle',
    error: null,
  });
  ...
  load(): void {
    ...
    this.http.get<SkillsData>('assets/data/skills.json').subscribe({
      next: (data) => this.state.set({ data, state: 'success', error: null }),
      error: (err) =>
        this.state.set({
          data: null,
          state: 'error',
          error: err.message ?? 'Error al cargar habilidades',
        }),
    });
  }
}
